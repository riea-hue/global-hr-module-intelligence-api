...
router = APIRouter(prefix="/api/v1/workers", tags=["Core HR / Organization"])
...
@router.get("/{record_id}", response_model=Workers, summary="Get workers by ID")
def get_workers_by_id(
    ...
    if record.empty:
        raise HTTPException(status_code=404, detail="workers record not found")
