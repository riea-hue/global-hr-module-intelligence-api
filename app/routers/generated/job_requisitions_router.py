...
router = APIRouter(prefix="/api/v1/job_requisitions", tags=["Recruiting"])
...
@router.get(
    "/{record_id}", response_model=JobRequisitions, summary="Get job_requisitions by ID"
)
def get_job_requisitions_by_id(
    ...
    if record.empty:
        raise HTTPException(status_code=404, detail="job_requisitions record not found")
