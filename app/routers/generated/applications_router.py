...
from app.models.generated.applications_model import Applications
...
router = APIRouter(prefix="/api/v1/applications", tags=["Recruiting"])

require_access = partial(enforce_department_access, "applications")
...
def get_applications(
    ...
    df = load_csv_data("applications.csv")
...
@router.get(
    "/{record_id}", response_model=Applications, summary="Get applications by ID"
)
def get_applications_by_id(
    ...
    df = load_csv_data("applications.csv")
    record = df[df["application_id"] == record_id]

    if record.empty:
        raise HTTPException(status_code=404, detail="applications record not found")
