...
from app.models.generated.locations_model import Locations
...
router = APIRouter(prefix="/api/v1/locations", tags=["Core HR / Organization"])

require_access = partial(enforce_department_access, "locations")
...
def get_locations(
    ...
    df = load_csv_data("locations.csv")
...
@router.get("/{record_id}", response_model=Locations, summary="Get locations by ID")
def get_locations_by_id(
    record_id: str,
    access_context: dict = Depends(require_access),
):
    df = load_csv_data("locations.csv")
    record = df[df["location_id"] == record_id]

    if record.empty:
        raise HTTPException(status_code=404, detail="locations record not found")
