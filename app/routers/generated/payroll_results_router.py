...
router = APIRouter(prefix="/api/v1/payroll_results", tags=["Payroll"])
...
@router.get(
    "/{record_id}", response_model=PayrollResults, summary="Get payroll_results by ID"
)
def get_payroll_results_by_id(
    ...
    if record.empty:
        raise HTTPException(status_code=404, detail="payroll_results record not found")
