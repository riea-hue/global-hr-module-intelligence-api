...
from app.models.generated.skills_model import Skills
...
router = APIRouter(prefix="/api/v1/skills", tags=["Skills Cloud"])

require_access = partial(enforce_department_access, "skills")
...
@router.get("/{record_id}", response_model=Skills, summary="Get skills by ID")
def get_skills_by_id(
    record_id: str,
    access_context: dict = Depends(require_access),
):
    df = load_csv_data("skills.csv")
    record = df[df["skill_id"] == record_id]
