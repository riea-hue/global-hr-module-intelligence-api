...
from app.models.generated.job_families_model import JobFamilies
...
router = APIRouter(prefix="/api/v1/job_families", tags=["Job Architecture"])

require_access = partial(enforce_department_access, "job_families")


@router.get(
    "",
    summary="Get job_families",
...
def get_job_families(
    ...
    df = load_csv_data("job_families.csv")
...
@router.get(
    "/{record_id}", response_model=JobFamilies, summary="Get job_families by ID"
)
def get_job_families_by_id(
    record_id: str,
    access_context: dict = Depends(require_access),
):
    df = load_csv_data("job_families.csv")
    record = df[df["job_family_id"] == record_id]

    if record.empty:
        raise HTTPException(status_code=404, detail="job_families record not found")
